...
for(f in 1:cvCount){
  trainingData = creditTrainFS[folds[[f]],]
  testData = creditTrainFS[-folds[[f]],]
  ...
  #show(best)
  #show("Train Accuracy:")
  #show(model_dectree$results[best,3])
  train.accuracy.estimate[f] = as.numeric(model_dectree$results[best,3])
  #show("Test Accuracy:")
  #show((table(predict(model_dectree,testData),testData$class)[1,1]+table(predict(model_dectree,testData),testData$class)[2,2])/length(testData$class))
  fold.accuracy.estimate[f] = (table(predict(model_dectree,testData),testData$class)[1,1]+table(predict(model_dectree,testData),testData$class)[2,2])/length(testData$class)
}
mean(train.accuracy.estimate)
mean(fold.accuracy.estimate)
...
#read dataset
#work
#projDS <- fread('C:/Users//m039388/Desktop/546/train.tsv', stringsAsFactors = F, nrows=1000)
#home
numberRows <- 150000
...
#projDS <- fread('C:/Users//m039388/Desktop/546/train.tsv', stringsAsFactors = F, nrows=numberRows)
projDS <- fread('C:/Users//franz_000/Desktop/546/train.tsv', stringsAsFactors = F, nrows=numberRows)
summary(projDS)

summary(projDS$price)